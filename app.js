        ...
        try {
            ...
            const carList = cars.map(car => `
                <div class="car-card" id="car-${car.id}">
                    ...
                    <button onclick="initiateBooking(${car.id}, ${car.availability}, '${car.title}', ${car.price})">
                        Book Now
                    </button>
                </div>
            `).join('');
...
async function bookCar(carId, carTitle, pricePerDay) {
    ...
    try {
        await createBooking(carId, startDatetime, endDatetime, totalCost);
...
async function createBooking(carId, startDatetime, endDatetime, totalCost) {
...
async function updateCarAvailability(carId, availability) {
    ...
    if (!response.ok) {
        throw new Error('Did not update car availability');
    }
